 ...
 *  gets (grassy blades / craggy stone / sandy grit / cobbled paving). Kept in sync with the editor. */
...
export function colorForBiome(biome: number, h: number, _wx: number, _wz: number, out: THREE.Color): THREE.Color {
  ...
  switch (biome) {
    ...
    case 20: { const sn = THREE.MathUtils.clamp((h - 26) / 20, 0, 1); return out.copy(PALETTE.mountainLow).lerp(PALETTE.mountainHigh, THREE.MathUtils.clamp(h / 40, 0, 1)).lerp(PALETTE.snow, sn); } // Mountains
