...
// entity count, and sim steps. Planned devtool per docs/qa/TEST_STRATEGY.md.
// Excluded from production builds in a later phase; always on during early dev.
...
export class PerfOverlay {
  ...
  update(frameMs: number, drawCalls: number, entities: number, steps: number): void {
    const instFps = frameMs > 0 ? 1000 / frameMs : 0;
    this.fpsEma += (instFps - this.fpsEma) * 0.1;
    this.el.textContent =
      `Oathbound 0.0.2-INDEV\n` +
