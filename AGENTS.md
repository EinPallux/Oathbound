...
- **Current build: `0.7.0-INDEV`** (0.6.0 feature-complete; **0.7.0 "Feel & Finish" in progress**). Everything `0.0.1` → `0.5.0`, plus `0.6.0` **CP1 (Legendary)** and the owner-requested **world/biome/player-model pass**, is **merged** into `claude/game-design-docs-70dim2`. The rest is a **stacked branch chain awaiting PR**: `0.6.0` **CP2 (world bosses)** `…-world-bosses` → **CP3 (Relics)** `…-relics` → **CP4 (Lv-30 endgame)** `…-endgame` → `0.7.0` **CP1 (Settings & accessibility)** `claude/oathbound-0.7.0-feel-finish` (each verified). The full game is playable end-to-end: 3 classes, 6 regions, the grind/loot/equip loop, Epic + Legendary + **Relic** rarity, Reinforcement + BLP, the Lv-30 capstone, three telegraphed world bosses, hand-designed relics, a Lv-30 endgame chase with target-farming guidance, and a **persisted Settings/accessibility panel (`O`)**.
...
- **Health (last full check):** `typecheck` ✓ · **287/287** unit ✓ · `build` ✓ (~179 KB gzip) · **15/15** e2e ✓ · no conflict markers / code smells.
