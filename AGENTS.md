...
- **Current build: `0.7.0-INDEV`** (0.6.0 **and** 0.7.0 feature-complete; awaiting playtest). Everything `0.0.1` → `0.5.0`, plus `0.6.0` **CP1 (Legendary)** and the owner-requested **world/biome/player-model pass**, is **merged** into `claude/game-design-docs-70dim2`. The rest is a **stacked branch chain awaiting PR** (each verified): `0.6.0` **CP2 (world bosses)** `…-world-bosses` → **CP3 (Relics)** `…-relics` → **CP4 (Lv-30 endgame)** `…-endgame` → `0.7.0` **CP1 (Settings/accessibility)** `…-feel-finish` → **CP2 (tooltips)** `…-tooltips` → **CP3 (audio+vignette)** `…-audio-vfx` → **CP4 (remappable controls)** `claude/oathbound-0.7.0-controls`. The full game is playable end-to-end: 3 classes, 6 regions, the grind/loot/equip loop, Epic + Legendary + **Relic** rarity, Reinforcement + BLP, the Lv-30 capstone, three telegraphed world bosses, hand-designed relics, a Lv-30 endgame chase with guidance, and the full **Settings/accessibility** suite (UI scale, reduced effects, colorblind-safe rarity, item tooltips+comparison, audio volume/mute, **remappable keybinds + mouse options**) — all persisted & live.
...
- **Health (last full check):** `typecheck` ✓ · **293/293** unit ✓ · `build` ✓ (~180 KB gzip) · **16/16** e2e ✓ · no conflict markers / code smells.
