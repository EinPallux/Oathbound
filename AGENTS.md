...
- **🚧 In progress — `0.5.0` Road to Thirty (Lv 21–30)** (checkpoints). **CP1 done:** the final two regions — **The Riven Peaks (east, Lv 21–25, frost)** and **Gravereach (north, Lv 26–30, undead/Blight)** — six families, the **frost** school + **+Frost Resist** affix, and the **undead holy-weakness lever**. **CP2 done:** the **Lv-30 capstone** — owner chose *upgrade-style* (no new button); at 30 each class's signature spender is empowered in place (Warrior Whirl→Oathbreaker's Wrath, Hunter Piercing Arrow→Rapid Fusillade, Priest Searing Light→Dawnbreak) via `Capstone`/`empowerAbility` in `src/sim/classes.ts`. **Remaining:** CP3 full 1→30 validation → Level 1–30 Content Gate.
- **⏳ Awaiting owner playtest:** (a) the standing **0.4.0** Lv 11–20 experience (zones + Epic/Reinforcement/BLP, PR #14); (b) **0.5.0 CP1+CP2** — the two endgame zones + the Lv-30 capstone feel good (the empowered spender should be a satisfying "I hit 30" spike).
- **▶ Next checkpoint (CP3):** a full **1 → 30 validation** pass — play/verify each class reaches 30 with no blocker and reasonable pacing across all six zones; tighten any rough spot → the **Level 1–30 Content Gate**. (Confirm with the owner before starting.)
- **Branching note:** parallel sessions build on feature branches → PRs into `claude/game-design-docs-70dim2`. Open PRs: **#14** (0.4.0 CP5) and **#15** (0.5.0, this branch `claude/oathbound-0.5.0-road-to-thirty`, **stacked on #14** — now holds CP1+CP2). Merge #14 first, then #15.
