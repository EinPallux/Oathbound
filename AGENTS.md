...
- **🚧 In progress — `0.5.0` Road to Thirty (Lv 21–30)** (checkpoints). **CP1 done:** the final two regions — **The Riven Peaks (east, Lv 21–25, frost)** and **Gravereach (north, Lv 26–30, undead/Blight)** — six new data-driven families, the **frost** damage school + a **+Frost Resist** affix, and the **undead holy-weakness lever** (Priest shines in the endgame zones); per-zone Oathstones + Goal-Tracker guidance. **Remaining:** CP2 Lv-30 capstone / kit-to-30 (design call — hotbar is full at 10) · CP3 full 1→30 validation → Level 1–30 Content Gate.
- **⏳ Awaiting owner playtest:** (a) the standing **0.4.0** Lv 11–20 experience (zones + Epic/Reinforcement/BLP chase, from PR #14); (b) **0.5.0 CP1** — the two endgame zones feel right (frost resist earns its keep in Riven; Priest's holy lever vs the Gravereach undead; the elite camps).
- **▶ Next checkpoint (CP2):** the Lv-30 **capstone / kit-to-30**. Note the kit is **8 base abilities + 2 choice nodes = 10 hotbar slots, full by Lv 18** — so the capstone is a *design decision*: upgrade an existing slot, add a third choice node, or grow the hotbar. **Confirm the approach with the owner before building.**
- **Branching note:** parallel sessions build checkpoints on separate feature branches → PRs into `claude/game-design-docs-70dim2`. Open PRs: **#14** (0.4.0 CP5) and **0.5.0 CP1** (this branch, `claude/oathbound-0.5.0-road-to-thirty`, **stacked on #14**). Merge #14 first, then rebase/merge 0.5.0.
