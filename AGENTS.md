...
- **Health (last full check):** `typecheck` ✓ · **276/276** unit ✓ · `build` ✓ (~176 KB gzip) · **14/14** e2e ✓ · no conflict markers / code smells.
...
- **🚧 In progress — `0.7.0` "Feel & Finish" (UX, accessibility & content polish)** — gameplay **feature-freeze**; spec [`docs/design/UX_AND_ACCESSIBILITY.md`](./docs/design/UX_AND_ACCESSIBILITY.md). **CP1 done (this branch, stacked on 0.6.0):** a **persisted Settings panel (`O`)** applied live (no restart) via CSS vars/classes (`src/game/settings.ts` + `src/render/settings-panel.ts`) — UI scale, damage-number toggle/size, reduced effects (motion & flashing), high-contrast rarity — plus **colorblind-safe rarity baseline** (text tier tag `[C/U/R/E/L/★]` + per-rarity border *shape* in the inventory + on loot toasts; "never colour-only"). **Remaining:** CP2 tooltips & item comparison + Character/Skills panels + destructive-action confirms · CP3 audio pass (Howler) + volume sliders + VFX/juice within budgets · CP4 fully remappable keybinds + camera/mouse options + onboarding polish + remaining HUD. **▶ Next checkpoint: CP2 (tooltips & comparison)** — confirm with the owner before starting. (Then 0.8.x optimization/balance → 0.9.x RCs → 1.0-BETA.) **Open item:** the 0.6.0→0.7.0 branch chain awaits PR/merge.
