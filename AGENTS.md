...
- **🚧 In progress — `0.6.0` "The Chase" (equipment depth + Lv-30 endgame)** → the **Endgame Foundation Gate** (spec: [`docs/design/ENDGAME_FOUNDATION.md`](./docs/design/ENDGAME_FOUNDATION.md)). **CP1 done:** **Legendary** rarity (orange, 4 affixes, ×1.45; ~6% tail on rare-named / ~1.5% on elites; counts as rare+ for BLP; rarity-coloured loot toasts). **Remaining:** CP2 the **3 solo world bosses** (multi-phase, telegraphed; Legendary/Relic loot) · CP3 **Relics** (hand-designed unique items + effect hooks) · CP4 Lv-30 endgame loop + Endgame Foundation Gate validation. **▶ Next checkpoint: CP2 (world bosses)** — confirm with the owner before starting.
