...
- **Preset asset library** (`src/world/presets.ts`, a mirror of the builder's): ~30 data-driven props (houses, cottage, two-story, longhouse, round/square/watch/wall towers, walls, gate, fence, palisade, well, market stall, tent, ruins, plus more trees, rocks, crystals and plants). Placed as `preset:<id>` and built through the shared custom-asset geometry, so they render identically to the editor.
