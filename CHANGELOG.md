...
- **Phase 1 — VERY HIGH (crash / DoS / data-integrity):** guarded `applyDamage` against a destroyed source/target (a trap/projectile outliving its caster no longer throws in the tick loop) + wrapped the tick in try/catch + top-level `uncaughtException`/`unhandledRejection` handlers; fixed the per-IP cap collapsing to one global bucket behind Caddy (resolve the real client IP from `X-Forwarded-For`), so the server no longer hard-caps at 8 total players; async scrypt + a strict auth rate budget (unauthenticated register/login can't stall the tick); `deleteChar` refused while a character is in-world (was a data-corruption path); real value-validation on imported saves (NaN position / absurd stats / huge blobs rejected) + `maxPayload`.
- **Phase 2 — HIGH (multiplayer correctness, leaks, UX):** boss ground-AoE now hits **every** player in the radius (was only the first); Oathstones/respawn work for every player and are tracked **per-player** (new `WaypointUnlocks` component — unlocks no longer leak between characters via saves); boss HP scaling resets on leash (no permanently-inflated boss for the next soloist); quest progress preserved across server flush + `Game.save()`; loot/trap meshes dispose their GPU resources; held keys clear on window blur (no more auto-run after Alt-Tab).
- **Phase 3 — MEDIUM (perf, robustness, online UX, docs):** camera terrain-collision is now **analytic** (heightfield march) instead of raycasting the 100k-tri terrain every frame; expired sessions purged; `enterImport` writes the DB row before spawning (no phantom orphan); login is constant-time against username enumeration; a **local health bar** in the online client (no more dying blind); online self-omission guard + capped prediction buffer; `bootOnline` teardown registry; rewrote a badly-stale README and synced the version to `0.8.0-ONLINE.7`; supersession banners on pre-MMO technical docs.
- **Phase 4 — LOW + cleanup:** dead players can't act during the respawn window; `deadTimers` pruned; protocol `ability`/`seq` bounded; de-duplicated `PICKUP_RADIUS`; perf overlay is dev-only now; moved ~21 MB of unused reference PNGs out of `public/` (they were shipping in every deploy); relocated the root authoring guides into `docs/guides/`; removed dead code (`createWisp`, unused `GameState` members).

**Verified (each phase):** client + server `typecheck` ✓ · unit **364/364** ✓ (added apply-guard, per-player-travel, boss-AoE/leash regressions) · `server:test` ✓ · `build` + `server:build` ✓ · live ws smokes (crash-safety, async auth, deleteChar/import guards, admin flows) ✓ · **e2e at the documented 9-pass baseline** throughout. Deferred (noted, not bugs): per-tick allocation micro-opts, offline-only telemetry map, `vercel.json` (owner decision on whether Vercel stays a target), audio attribution in CREDITS.

---

## M7 "Ops & Hardening" — the VPS is a product now
**Goal (see [docs/production/MMO_ROADMAP.md](./docs/production/MMO_ROADMAP.md)):** the owner can stand up or update the server in minutes, and it survives reboots, crashes, and disk mishaps — plus in-world moderation and a real capacity measurement.
- **Admin commands** (in-chat, behind the account admin flag; auto-promoted from `OATHBOUND_ADMINS`): **`/admin broadcast <msg>`**, **`kick <name>`**, **`ban <name>`** (persisted — the banned account is refused at login), **`save`** (flush now), **`shutdown [s]`** (warn everyone, then graceful exit after the window), **`who`** (admin roster with account ids). Public **`/who`** lists online characters. A non-admin's `/admin` is politely refused.
- **Bans** (`002_bans` migration: `accounts.is_banned`): a ban disconnects the player immediately **and** blocks future logins (`code: 'banned'`), enforced in both `login` and `resume`.
- **Tick-time metrics:** `ServerClock` now measures `performance.now()` around each `step(DT)` and `takeStats()` feeds the 5 s heartbeat — `step avg X ms max Y ms (budget 33ms)` — so capacity is observable in the logs.
- **Bot load test** (`server/test/load.ts`, `npm run server:loadtest`): N headless bots authenticate, create/select a character, and drive random movement + occasional abilities against a running server, reporting downstream bandwidth; the server heartbeat reports the sim step time. Per-IP cap made configurable (`OATHBOUND_MAX_CONN_PER_IP`) so a local soak isn't self-throttled. **Measured (20 bots): step avg ~0.6 ms / max ~1.85 ms** vs the 33 ms budget (~2%), **~280 kbit/s per bot** (~5.6 Mbit/s aggregate). CPU is a non-issue at friends scale; **bandwidth is the first lever** (snapshots currently carry all entities — interest management is the deferred win).
- **Ops kit (`deploy/`):** `setup.sh` (one-time Ubuntu 24.04 provisioning — Node 22, Caddy, non-root service user, dirs, UFW, systemd units, backup timer), `deploy.sh` (`npm ci` → build client + server → `systemctl restart`), `Caddyfile` (auto-TLS, static `dist/` + `/ws` proxy), `oathbound.service` (auto-restart, graceful SIGTERM flush, `ReadWritePaths` sandbox), `server.env.example` (all config knobs), `backup.sh` + `sqlite-backup.mjs` (WAL-safe online backup → gzip → rotate, no system `sqlite3` needed), `restore.sh` (validated, keeps a `.pre-restore` copy), `oathbound-backup.{service,timer}` (nightly), and a `README`.
- **Deferred (documented):** interest-radius / delta snapshots (the bandwidth lever), Litestream replication, a CLI admin socket (in-chat is enough for friends scale), and teleport/give admin verbs (dev-tools parity — out of M7 scope).

**Verified:** `typecheck` ✓ (client + server) · `npm test` → **359/359** ✓ (server-side ops are transport/infra, not sim) · `server:test` ✓ (with the `002_bans` migration) · `build` ✓ (offline bundle unchanged) · `server:build` ✓ (231 kB bundle). **Backup/restore drill** (real WAL-hot DB via the `Db` class): online backup taken against a live DB → a subsequent write → `restore.sh` correctly rolled back to the backup's state (mutation gone, seeded character/world-state intact, `.pre-restore` copy kept). **Live admin verify (2-client `ws`): 12/12** — `/who`, `/admin who` (admins marked `*`), non-admin `/admin` refused, `broadcast` reached both, `save`, `kick` (notified + disconnected), `ban` (notified + disconnected + **login refused after**), `shutdown 1` (warned, then graceful exit). **e2e: parity with baseline** — 9/9 (the same 9 environment-limited render tests; no client behaviour changed in M7). **Next phase → M8 "Friends Beta → Online Gate"** (the final phase: real playtest with friends, tune, and clear the objective Online Gate) — confirm with the owner.
...
- **Shared XP:** on a kill, **every participant** (the final-blow killer + everyone who damaged it) earns **full XP** scaled to their own level — grouping doesn't dilute rewards. A dead-but-present participant still counts (assisting a kill is eligible).
- **Owner-instanced loot:** a shared kill spawns **one loot drop per participant**, each owned by that player (the M2 `LootDrop.owner` payoff) — no contested loot, by construction. The killer is paid out first, so a **solo kill is byte-for-byte identical** (same RNG draws, one drop) — proven by the unchanged loot/endgame tests.
- **World-boss HP scaling:** on the first engaged tick a boss scales its max HP by the number of players in the fight (base × (1 + 0.6·(n−1)); solo = base, restored on reset) — a 3-player pull is a real fight, not a walled or trivial one; mechanics/telegraphs are unchanged.
- **Deferred (documented):** healing threat (v1 is damage-only — healers naturally hold low aggro), DoT threat (ticks applied outside `applyDamage`), a formal party/grouping system (XP/loot share to nearby threat-participants for now), and boss *mechanic* scaling (only HP scales in v1).

**Verified:** `typecheck` ✓ (client + server) · `npm test` → **359/359** ✓ (new `multiplayer-rules`: an enemy targets the high-threat player and ignores the low-threat one; a shared kill gives both participants XP + one own-loot drop each; a solo kill gives exactly one drop owned by the killer; a boss is base HP solo and **base × 2.2** with three players) · `server:test` ✓ · `build` ✓ (offline bundle unchanged) · `server:build` ✓. The M5 rules run in the shared sim (`createSimWorld`) the server ticks, so the real-sim unit tests cover the server behaviour directly. **e2e: parity with baseline** — the same 9 environment-limited tests fail on this and pre-M5 code (incl. the boss-fight test), the offline combat/boss behaviour is unchanged (solo-neutral). **Next phase → M6 "Chat & Presence"** (chat overlay, join/leave + system messages, /who, MOTD) — confirm with the owner.

---

## M4 "Netcode Feel" — prediction, interpolation, reconnect, and abuse limits
**Goal (see [docs/production/MMO_ROADMAP.md](./docs/production/MMO_ROADMAP.md)):** hide latency so movement feels instant instead of server-echo-laggy, survive bad networks, and stop trusting the client's *volume*.
- **Client-side prediction + reconciliation** (`src/net/prediction.ts`, isomorphic): the local player runs the **same** movement code the server runs (`createMovementSystem` over the same heightfield + colliders — the client now builds them via `buildCustomWorldData`) on a one-entity world, so your own movement responds immediately. Each input is buffered with its `seq`; when an authoritative snapshot arrives, the client snaps to the server position and **replays** the un-acked inputs. Same code + same inputs → prediction matches the server, and a correction (knockback, a rejected move) is a small snap, not rubber-banding.
- **Remote interpolation buffer:** other entities render from a short **sample buffer at a fixed ~100 ms delay** (interpolating between the two bracketing snapshots) — smooth under 15 Hz snapshots + jitter, instead of snapping. The local player is sub-tick interpolated by the render `alpha`.
- **Reconnect / resume:** on disconnect the server **keeps the player's entity alive for a 20 s grace window** (input neutralized so it stands still), flushed to the DB; the same account re-selecting that character **re-attaches to the live entity in place** (same entity id, exact position) instead of reloading — plus a **duplicate-session guard** (a character can't be in the world twice). The client reloads to re-enter on an unexpected drop, landing back in place within the grace window.
- **Abuse limits (the client is untrusted):** a token-bucket **`RateLimiter`** (`src/net/rate-limit.ts`, isomorphic) per connection drops over-rate messages and disconnects a persistent flooder; **per-IP connection caps** reject excess sockets; `yaw` is clamped to **finite** in the protocol (no NaN/∞ poisoning the movement integration). The sim already enforces cooldowns/range/resources, so gameplay validation was free.
- **Deferred (recorded levers, unneeded at friends scale):** delta snapshots, interest management (area-of-interest culling), and a binary wire format (msgpack); ability/combat prediction (only movement is predicted — casts tolerate a round-trip).

**Verified:** `typecheck` ✓ (client + server) · `npm test` → **355/355** ✓ (new `prediction` test: reconciling to an agreed server state is stable, a correction snaps + clears acked inputs; new `rate-limit` test: burst-then-throttle, capacity cap) · `server:test` ✓ (persistence unchanged) · `build` ✓ (offline bundle unchanged) · `server:build` ✓. **Live end-to-end** (`ws` clients on a running server): **reconnect** re-attached to the **same entity (10 → 10) at the identical position (0.00 m drift)** within the grace window; the **per-IP cap** rejected excess connections with close code 1008; a **flooder** was rate-limit-closed (1008) while the server stayed alive for a fresh client. **e2e: parity with baseline** (offline behaviour-neutral — M4 only touched the net layer / server / online client). *(The browser prediction/interpolation feel is the owner's playtest — the reconciliation + rate-limit logic is unit-tested and the reconnect/cap/flood behaviour is verified headlessly.)* **Next phase → M5 "Playing Together"** (XP sharing, instanced loot rolls, multi-player threat, world-boss scaling) — confirm with the owner.

---

## M3 "Accounts + SQLite" — real accounts + durable server-side persistence
**Goal (see [docs/production/MMO_ROADMAP.md](./docs/production/MMO_ROADMAP.md)):** identity + durable storage. Everything the game persists now lives in **SQLite on the server**; a character survives a server restart.
- **SQLite** (`better-sqlite3`, WAL) in `server/db.ts`: ordered, transactional **migrations**; tables `accounts`, `sessions`, `characters` (hot columns + the full versioned **`SaveData` JSON** — reusing the game's `serialize()`/`applySave()` as the exact boundary), `world_state`, `meta`. Pragmas: WAL · `synchronous=NORMAL` · `foreign_keys=ON` · `busy_timeout`.
- **Auth** (`server/auth.ts`): passwords **scrypt**-hashed with a per-account salt; opaque session tokens stored only as a **SHA-256 hash** (a DB leak can't be replayed); constant-time verify. Optional server-wide **join password** (friends-only lock).
- **Connection flow** (`server/net.ts` + `game.ts`): connect → `register`/`login`/`resume` → `authOk` + `charList` → `selectChar`/`createChar`/`importChar` → **enter world** (`welcome` + snapshot). Only in-world does `input` flow. A connection loads its character with `applySave` onto a fresh player entity; per-account **3 character slots**.
- **Write-behind persistence:** in-world characters are flushed to the DB via `serialize()` **every 30 s**, **on disconnect**, and on **graceful `SIGTERM`/`SIGINT`** (drains everyone before exit). WAL keeps the file consistent.
- **Local-save import:** a one-time "import my offline character" — the browser uploads its IndexedDB `SaveData`, the server **structurally validates** it, then creates the character. (The long-promised offline→online migration path.)
- **Protocol** grew `register`/`login`/`resume`/`createChar`/`selectChar`/`deleteChar`/`importChar` (C→S) and `authOk`/`charList` (S→C), all zod-validated. **Client** (`src/game/online.ts`) gained a login form + character-select panel (create / play / delete / import), with `?server=&user=&pass=&char=&class=` URL auto-login; the 3D scene is built only after `welcome`. Offline mode keeps IndexedDB, untouched.
- **Deferred (documented):** `world_state` boss-timer persistence (table exists; bosses still reset on restart), a full zod schema for imported saves (a structural validator is used), and account management UI (password reset, etc.).

**Verified:** `typecheck` ✓ (client + server) · `npm test` → **351/351** ✓ (new `save-roundtrip`: serialize → JSON → parse → `applySave` preserves gold/level/inventory/position — the SQLite `save_json` boundary) · **`npm run server:test`** ✓ (new `server/test/persistence.test.ts`: DB repos, scrypt auth, sessions incl. expiry, and a character surviving a **close-and-reopen** of the DB file) · `build` ✓ (offline bundle unchanged) · `server:build` ✓ → the production bundle **boots with the external SQLite module**, creates the DB, and ticks 30 Hz. **Live full-stack restart:** registered an account, created a character, moved it to (−633.1, −134.3), disconnected; **killed and restarted the server process on the same DB file**; logged back in, selected the character — it loaded at (−633.1, −134.1), **0.22 m** from where it left off. **e2e: parity with baseline** (offline behaviour-neutral; the transient 10th failure on one run was CPU contention from a background test server, not a regression — the clean re-run matched the 9/9 baseline). **Next phase → M4 "Netcode Feel"** (client-side prediction + reconciliation, interpolation, reconnect, rate limits) — confirm with the owner.
