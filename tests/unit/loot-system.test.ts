...
import { createLootSystem } from '../../src/sim/systems/loot';
...
describe('loot system', () => {
  it('despawns uncollected drops after their TTL (no unbounded growth)', () => {
    ...
    const sys = createLootSystem({ input: ctrl });
  ...
  it('auto-collects gold and picks up items on interact', () => {
    const world = new World();
    ...
    state.interact = true;
    sys.update(world, DT); // F picks up the item
